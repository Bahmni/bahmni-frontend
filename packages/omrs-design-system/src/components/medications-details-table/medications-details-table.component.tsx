          ...
          <div className={styles.medicationRecord}>
            <div>
              <p className={styles.bodyLong01}>
                ...
                {medication.drug?.strength.toLowerCase()} &mdash;{' '}
              ...
              <p className={styles.bodyLong01}>
                ...
                {medication.frequency?.display.toLowerCase()} &mdash;{' '}
                {!medication.duration
                  ? t(
                      'medicationIndefiniteDuration',
                      'Indefinite duration',
                    ).toLowerCase()
                  : t(
                ...
                {medication.numRefills !== 0 && (
                  <span>
                    <span className={styles.label01}>
                      {' '}
                      &mdash; {t('refills', 'Refills').toUpperCase()}
                    </span>{' '}
                ...
                {medication.dosingInstructions && (
                  <span>
                    {' '}
                    &mdash; {medication.dosingInstructions.toLocaleLowerCase()}
                  </span>
                )}
              </p>
            </div>
            <p className={styles.bodyLong01}>
              ...
              {medication.quantity ? (
                <span>
                  <span className={styles.label01}>
                    {' '}
                    &mdash; {t('quantity', 'Quantity').toUpperCase()}
                  </span>{' '}
                  {medication.quantity}
                </span>
              ) : null}
              {medication.dateStopped ? (
                <span className={styles.bodyShort01}>
                  <span className={styles.label01}>
                    {medication.quantity ? ` — ` : ''}{' '}
                    {t('endDate', 'End date').toUpperCase()}
                  </span>{' '}
      ...
      startDate: {
        ...
        content: (
          <div className={styles.startDateColumn}>
            ...
            <InfoTooltip
              orderer={medication.orderer?.person?.display ?? '--'}
            />
          </div>
        ),
      },
