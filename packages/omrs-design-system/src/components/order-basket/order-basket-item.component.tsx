...
export default function OrderBasketItemTile({
    ...
    <div className={styles.orderBasketItemTile}>
      <p className={styles.clipTextWithEllipsis}>
        ...
        {orderBasketItem.isFreeTextDosage ? (
          <>
            <span className={styles.drugName}>
              {orderBasketItem.drug.concept.display}
            </span>
            <span className={styles.dosageInfo}>
              {' '}
              &mdash; {orderBasketItem.freeTextDosage}
            </span>
          </>
        ) : (
          <>
            ...
            <span className={styles.dosageInfo}>
              {' '}
              &mdash; {orderBasketItem.dosage.dosage} &mdash;{' '}
        ...
        <span className={styles.label01}>
          <span className={styles.doseCaption}>
            {t('dose', 'Dose').toUpperCase()}
          </span>{' '}
          <span className={styles.dosageLabel}>
            {orderBasketItem.dosage.dosage}
          </span>{' '}
          <span className={styles.dosageInfo}>
            &mdash; {orderBasketItem.route.name} &mdash;{' '}
            {orderBasketItem.frequency.name} &mdash;{' '}
        ...
        <br />
        <span className={styles.label01}>
          <span className={styles.indicationLabel}>
  ...
  return orderBasketItem.action === 'DISCONTINUE' ? (
    ...
    <ClickableTile
      role="listitem"
      className={
        isTablet ? styles.clickableTileTablet : styles.clickableTileDesktop
      }
